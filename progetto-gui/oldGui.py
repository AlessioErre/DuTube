...
def Dutube():
   ...
   def WmaGUI():
      def get_link():
         link= entry.get()
         #cmd= subprocess.call(["python3", path + "wma.py", link])
         cmd= subprocess.call(["php", path + "Wma.php", link])
      ...
      label = Label(new, text='Converti Video In WMA')
      label.config(font=('helvetica', 14))
      canvas.create_window(200, 25, window=label)
   ...
   Button(win, text="WMA", command=WmaGUI).place(relx=0.5,  rely=0.5, anchor=NW)
